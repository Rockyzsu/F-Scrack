...
PASSWORD_DIC = ['123456', 'admin', 'root', 'password', '123123', '123', '1', '{user}', '{user}{user}', '{user}1',
                ...
                'p@55w0rd', 'password!', 'p@ssw0rd!', 'password1', 'r00t', 'tomcat', 'apache', 'system', 'huawei',
                'zte']

REGEX = [['ftp', '21', '^220.*?ftp|^220-|^220 Service|^220 FileZilla'],
         ['telnet', '23', '^\\xff[\\xfa-\\xfe]|^\\x54\\x65\\x6c|Telnet'], ['mssql', '1433', ''],
         ['mysql', '3306', '^.\\0\\0\\0.*?mysql|^.\\0\\0\\0\\n|.*?MariaDB server'], ['postgresql', '5432', ''],
         ['redis', '6379', '-ERR|^\\$\\d+\\r\\nredis_version'], ['elasticsearch', '9200', ''],
         ['memcached', '11211', '^ERROR'], ['mongodb', '27017', '']]
...
class Crack():
    ...
    def mssql(self, user, pass_):  # author:hos@YSRC
        try:
            ...
            husername = str(binascii.b2a_hex(user))
            lusername = len(user)
            lpassword = len(pass_)
            ladd = len(self.ip) + len(str(self.port)) + 1
            hladd = hex(ladd).replace('0x', '')
            hpwd = str(binascii.b2a_hex(pass_))
            pp = binascii.b2a_hex(self.port)
            address = str(hh + b'3a' + pp)
    ...
    def mongodb(self, user, pass_):
        try:
            ...
            if "ismaster" in str(result):
                ...
                s.send(getlog_data)
                result = s.recv(1024)
                if "totalLinesWritten" in str(result):
                    return "unauthorized"
                else:
                    return 3
        except Exception as e:
...
def get_ac_ip(ip_list):
    try:
        ...
        logger.error('The current user permissions unable to send icmp packets')
        logger.error(e)
        return ip_list
...
def log(scan_type, host, port, info=''):
    ...
    if scan_type == 'portscan':
        logger.info("[%s] %s:%d open" % (time_str, host, int(port)))
    elif scan_type == 'discern':
        logger.info("[%s] %s:%d is %s" % (time_str, host, int(port), info))
    elif scan_type == 'active':
        logger.info("[%s] %s active" % (time_str, host))
    elif info:
